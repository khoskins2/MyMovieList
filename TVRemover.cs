...
public class TVRemover
{
    public SkillResponse FunctionHandler(SkillRequest input, ILambdaContext context)
    {
        ...
        var itemNameSlot = intentRequest.Intent.Slots["tv_show"];
        ...
        if (intentRequest.DialogState == DialogState.Started || intentRequest.DialogState == DialogState.InProgress)
        {
            if (confirmationStatus == "CONFIRMED")
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    var response = new SkillResponse
                    {
                        ...
                        Response = new ResponseBody
                        {
                            OutputSpeech = new PlainTextOutputSpeech { Text = $"The tv show, {itemToDelete} has been deleted from the list, {listName}" },
                            ShouldEndSession = true,
                        }
                    };
                    return response;
            ...
            else if (confirmationStatus == "DENIED")
            {
                var response = new SkillResponse
                {
                    ...
                    Response = new ResponseBody
                    {
                        OutputSpeech = new PlainTextOutputSpeech { Text = $"The tv show, {itemToDelete} was not deleted." },
                        ShouldEndSession = true,
                    }
                };
                return response;
